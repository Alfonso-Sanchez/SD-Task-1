# Documentación: Práctica PYRO4 MultiNode (3 Servidores)

Esta documentación explica cómo configurar y ejecutar un proyecto basado en Pyro4 con tres servidores de insultos (`InsultServer1`, `InsultServer2`, `InsultServer3`), un cliente que envía insultos a los tres servidores (`Broadcaster`), un visualizador de estadísticas (`StatsDisplay`) y un conjunto de pruebas automatizadas (`TestInsultFilter`). El proyecto utiliza un entorno virtual de Python y requiere que el Name Server de Pyro4 esté activo.
...
El proyecto tiene cinco componentes principales: tres servidores (`InsultServer1`, `InsultServer2`, `InsultServer3`), el cliente (`Broadcaster`) y el visualizador de estadísticas (`StatsDisplay`). Sigue este orden para ejecutarlos.
...
#### Servidor 3 (`InsultServer3`)
En otra terminal (con el entorno activado), ejecuta:
```bash
python insult_server3.py
```
- `insult_server3.py` es el nombre sugerido para el tercer servidor.
- Se registra como `example.insultserver3` en el Name Server.

#### Notas
- Mantén las tres terminales abiertas mientras usas el cliente, el visualizador o las pruebas.

### 2.2. Ejecutar el Cliente (`Broadcaster`)
El cliente envía insultos aleatorios a uno de los tres servidores de forma continua.
...
- Usa `Ctrl+C` para detener el cliente y mostrar los insultos guardados en los tres servidores.

#### Notas
- El cliente elige aleatoriamente entre `example.insultserver1`, `example.insultserver2` y `example.insultserver3`.
...
El visualizador muestra las métricas de los tres servidores en una interfaz curses.
...
- Verás estadísticas como insultos por segundo, cada 10 segundos y por minuto para los tres servidores, junto con un total por segundo.
...
El archivo de pruebas (`TestInsultFilter`) verifica el comportamiento de los tres servidores. Requiere que todos estén corriendo.

### Paso 1: Asegurarse de que los servidores estén activos
Ejecuta `insult_server1.py`, `insult_server2.py` y `insult_server3.py` como se describe en la sección 2.1.
...
- **Pruebas para Server1, Server2 y Server3** (duplicadas para cada servidor):
...
### Notas adicionales
- Las pruebas usan `clearSentences()` para limpiar los servidores antes de cada prueba. Asegúrate de que los servidores estén recién iniciados o que este método funcione.
...
- **Estructura sugerida**:
  ...
  - `insult_server3.py`: Tercer servidor.
...
- **Flujo típico**:
  ...
  2. Inicia los tres servidores (`python insult_server1.py`, `python insult_server2.py`, `python insult_server3.py`).
