...
def recibir_insultos():
    ...
    channel.queue_declare(queue='insultos2')
    ...
    channel.basic_consume(queue='insultos2', on_message_callback=callback, auto_ack=True)
...
def filtrar_frases():
    ...
    connection = pika.BlockingConnection(pika.ConnectionParameters(host='localhost'))
    channel = connection.channel()
    channel.queue_declare(queue='filtrar2')
    channel.queue_declare(queue='filtradas2')
    ...
    def callback(ch, method, properties, body):
        ...
        ch.basic_publish(exchange='', routing_key='filtradas2', body=resultado)

    channel.basic_consume(queue='filtrar2', on_message_callback=callback, auto_ack=True)
    channel.start_consuming()
...
def responder_insultos():
    ...
    connection = pika.BlockingConnection(pika.ConnectionParameters(host='localhost'))
    channel = connection.channel()
    channel.queue_declare(queue='insultame2')
    channel.queue_declare(queue='insultar2')
    ...
    def callback(ch, method, properties, body):
        ...
        ch.basic_publish(exchange='', routing_key='insultar2', body=insulto)

    channel.basic_consume(queue='insultame2', on_message_callback=callback, auto_ack=True)
    channel.start_consuming()
...
def mostrar_stats(stdscr):
    ...
    while True:
        ...
        stdscr.clear()
        stdscr.addstr(1, 1, "== Servidor 2 RabbitMQ - Estadísticas ==")
...
if __name__ == '__main__':
    ...
    threading.Thread(target=publicar_stats, args=("Servidor2",), daemon=True).start()
