# Documentación: Práctica RabbitMQ Multi-Node (3 Nodos)

Esta documentación explica cómo configurar y ejecutar un proyecto basado en RabbitMQ con tres nodos servidores (`server1.py`, `server2.py`, `server3.py`), un broadcaster de insultos (`broadcaster.py`), un cliente que envía y recibe frases filtradas (`client_filter.py`), un cliente que solicita insultos (`insult_client.py`), un monitor centralizado de estadísticas (`stats_monitor.py`) y pruebas automatizadas (`test_servers.py`). El sistema usa un entorno virtual de Python y RabbitMQ como sistema de mensajería.
...
El proyecto tiene siete componentes ejecutables: tres servidores (`server1.py`, `server2.py`, `server3.py`), el broadcaster (`broadcaster.py`), el cliente de filtrado (`client_filter.py`), el cliente de insultos (`insult_client.py`) y el monitor de estadísticas (`stats_monitor.py`). Sigue este orden para ejecutarlos.
...
#### Servidor 3 (`server3.py`)
Procesa insultos de `insultos3`, filtra frases de `filtrar3` y responde solicitudes de `insultame3`.

En otra terminal (con el entorno activado), ejecuta:
```bash
python server3.py
```
- Escucha `insultos3`, `filtrar3` y `insultame3`, publica en `filtradas3`, `insultar3` y `stats`.
- Muestra estadísticas locales con curses.
- Usa `Ctrl+C` para detenerlo.

### 2.2. Ejecutar el Broadcaster (`broadcaster.py`)
Envía insultos aleatorios a las colas `insultos1`, `insultos2` y `insultos3`.
...
Envía frases a `filtrar1`, `filtrar2` o `filtrar3` y recibe las versiones filtradas de `filtradas1`, `filtradas2` o `filtradas3`.
...
Solicita insultos a `insultame1`, `insultame2` o `insultame3` y recibe respuestas de `insultar1`, `insultar2` o `insultar3`.
...
```
- Solicita insultos cíclicamente a uno de los servidores y muestra las respuestas.
- Usa `Ctrl+C` para detenerlo.

### 2.5. Ejecutar el Monitor de Estadísticas (`stats_monitor.py`)
Muestra estadísticas centralizadas de los tres servidores desde la cola `stats`.
...
```
- Muestra estadísticas de "Servidor1", "Servidor2" y "Servidor3" en una interfaz curses, incluyendo un total de peticiones por segundo.
- Usa `Ctrl+C` para detenerlo.
...
El archivo de pruebas (`test_servers.py`) verifica el comportamiento de los tres servidores con conexiones reales a RabbitMQ.

### Paso 1: Asegurarse de que los servidores estén corriendo
Ejecuta `server1.py`, `server2.py` y `server3.py` como se describe en la sección 2.1.
...
- **Pruebas para Server3**:
  - Solicitar un insulto desde `insultame3` y verificar la respuesta.
  - Filtrar una frase con insulto desde `filtrar3`.
  - Filtrar una frase sin insulto desde `filtrar3`.

### Notas adicionales
- Las pruebas pre-pueblan las colas `insultos1`, `insultos2` y `insultos3` con datos de prueba.
...
- **Estructura sugerida**:
  - `broadcaster.py`: Envía insultos a los tres servidores.
  - `client_filter.py`: Envía y recibe frases filtradas de los tres servidores.
  - `server1.py`: Servidor 1 (procesa `insultos1`, `filtrar1`, `insultame1`).
  - `server2.py`: Servidor 2 (procesa `insultos2`, `filtrar2`, `insultame2`).
  - `server3.py`: Servidor 3 (procesa `insultos3`, `filtrar3`, `insultame3`).
  - `insult_client.py`: Solicita insultos a los tres servidores.
  - `stats_monitor.py`: Monitor centralizado de estadísticas.
  - `test_servers.py`: Pruebas para los tres servidores.
  - `requirements.txt`: Dependencias optimizadas.
- **Flujo típico**:
  1. Inicia RabbitMQ con Docker (`docker run ...`).
  2. Inicia los tres servidores (`python server1.py`, `python server2.py`, `python server3.py`).
