...
def recibir_insultos():
    ...
    channel.queue_declare(queue='insultos3')
    ...
    channel.basic_consume(queue='insultos3', on_message_callback=callback, auto_ack=True)
...
def filtrar_frases():
    ...
    connection = pika.BlockingConnection(pika.ConnectionParameters(host='localhost'))
    channel = connection.channel()
    channel.queue_declare(queue='filtrar3')
    channel.queue_declare(queue='filtradas3')
    ...
    def callback(ch, method, properties, body):
        ...
        ch.basic_publish(exchange='', routing_key='filtradas3', body=resultado)

    channel.basic_consume(queue='filtrar3', on_message_callback=callback, auto_ack=True)
    channel.start_consuming()
...
def responder_insultos():
    ...
    connection = pika.BlockingConnection(pika.ConnectionParameters(host='localhost'))
    channel = connection.channel()
    channel.queue_declare(queue='insultame3')
    channel.queue_declare(queue='insultar3')
    ...
    def callback(ch, method, properties, body):
        ...
        ch.basic_publish(exchange='', routing_key='insultar3', body=insulto)

    channel.basic_consume(queue='insultame3', on_message_callback=callback, auto_ack=True)
    channel.start_consuming()
...
def mostrar_stats(stdscr):
    ...
    while True:
        ...
        stdscr.clear()
        stdscr.addstr(1, 1, "== Servidor 3 RabbitMQ - Estadísticas ==")
...
# Servicio 5: Centralizar las estadisticas 
...
if __name__ == '__main__':
    ...
    threading.Thread(target=publicar_stats, args=("Servidor3",), daemon=True).start()
