...
def mostrar_stats(stdscr):
    ...
    while True:
        ...
        total_solicitudes_segundo = 0
        for servidor in sorted(stats_data.keys()):
            datos = stats_data[servidor]
            total_solicitudes_segundo += datos.get('solicitudes/segundo', 0)

        try:
            stdscr.addstr(row, 4, f"Total peticiones por segundo (3 servidores): {total_solicitudes_segundo}")
        except curses.error:
            pass  # por si la pantalla es demasiado pequeña
