...
def recibir_insultos():
    ...
    channel.basic_consume(queue='insultos', on_message_callback=callback, auto_ack=True)
...
def filtrar_frases():
    ...
    def callback(ch, method, properties, body):
        ...
        ch.basic_publish(exchange='', routing_key='filtradas', body=resultado)

    channel.basic_consume(queue='filtrar', on_message_callback=callback, auto_ack=True)
    channel.start_consuming()
...
def responder_insultos():
    ...
    connection = pika.BlockingConnection(pika.ConnectionParameters(host='localhost'))
    channel = connection.channel()
    channel.queue_declare(queue='solicitudes_insulto')
    ...
    channel.basic_consume(queue='solicitudes_insulto', on_message_callback=callback, auto_ack=True)
    channel.start_consuming()

# Servicio 4: Mostrar estadísticas usando curses
def mostrar_stats(stdscr):
    global peticiones_en_segundo_actual, contador_insultos, contador_filtrados, contador_solicitudes_insulto
    curses.curs_set(0)
    stdscr.nodelay(True)
    while True:
        with lock:
        ...
        stdscr.clear()
        stdscr.addstr(1, 1, "== Servidor RabbitMQ - Estadísticas ==")
